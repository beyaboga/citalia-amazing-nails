...
export async function POST(request: Request) {
  ...
  const specialRequirements = body?.specialRequirements ? String(body.specialRequirements).trim() : null;
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO services (category_id, name, description, price, duration_minutes, is_active, special_requirements, image_url, image_alt)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       RETURNING
         id, category_id AS "categoryId", name, description, price,
      ...
      [categoryId, name, description, price, durationMinutes, isActive, specialRequirements, imageUrl, imageUrl ? name : null]
    );
